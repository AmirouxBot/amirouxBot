...
const langData = {
    ...
    "ar_SY": {
        ...
        "invalidQuery": "استعلام غير صالح ، الكلمات المتاحة: update, reset.",
...
async function resetConfirm({ message, eventData, getLang }) {
    ...
    try {
        ...
        message.send(getLang("تم اعادة التشغيل بنجاح"));
    } catch {
        message.send(getLang("خطأ"));
    }
}
...
function chooseReset({ message, getLang }) {
    ...
    if (isNaN(choice)) return reply(getLang("اختيار غير صالح"));
    if (choice < 1 || choice > 3) return reply(getLang("اختيار غير صالح"));

    const chosen = choice == 1 ? "threads" : choice == 2 ? "users" : "all";
    const type = global.config.DATABASE;

    reply(getLang(chosen) + getLang("تأكيد اعادة التشغيل"))
...
async function onCall({ message, args, getLang }) {
    ...
    switch (query) {
        case 'update':
            {
                ...
                message.reply(getLang("تم التحديث بنجاح"));
                break;
            }
        case 'reset':
            {
                message
                    .reply(getLang("اختر اعادة التشغيل"))
        ...
        default:
            {
                message.reply(getLang("استعلام غير صالح"));
                break;
            }
    }
